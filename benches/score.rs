...
fn bench_score(c: &mut Criterion) {
    ...
    for num_entries in [10, 100, 1000] {
        // Pre-compress all entries
        let mut all_compressed = Vec::new();
        for _ in 0..num_entries {
            let keys = random_vec(32 * d, &mut rng); // 32 vectors per entry
            ...
            |b, _| {
                b.iter(|| {
                    ...
                    for compressed in &all_compressed {
                        let scores = sketch
                            .score(black_box(&token), black_box(compressed))
                            .unwrap();
...
fn bench_score_single(c: &mut Criterion) {
    ...
    c.bench_function("score_single_entry_64vec", |b| {
        b.iter(|| sketch.score(black_box(&token), black_box(&compressed)));
    });
}
