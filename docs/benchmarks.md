...
## CPU Scoring (d=128, s=256, 32 vec/entry)

| Operation             | Time    |
| --------------------- | ------- |
| Score 1 entry (64 vec) | 24 us   |
| Score 10 entries        | 178 us  |
| Score 100 entries       | 1.8 ms  |
| Score 1000 entries      | 17.8 ms |

Linear scaling. ~18 us per entry, ~0.56 us per vector.
...
- At 10K entries: **7x speedup** (d=64) to **14.6x speedup** (d=128)

### When GPU helps

GPU is beneficial when:
1. Total vectors across all entries >= 5000 (default threshold)
