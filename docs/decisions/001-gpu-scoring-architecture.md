...
per-call, and from `scores()` per-entry. Benchmarks on
...
A single entry has 32 vectors. GPU overhead (2.1 ms) vs CPU cost
(32 × 0.33 µs = 10.6 µs for float×sign, 32 × 7 ns = 224 ns for
compressed). GPU is 200-9000x slower per entry.

### Per-entry store scoring — GPU makes it worse

| Entries | CPU compressed | GPU per-entry float×sign |
|---------|---------------|--------------------------|
| 10 | 18 µs | 20.5 ms (1000x slower) |
| 100 | 97 µs | 203 ms (2000x slower) |
| 1000 | 7.8 ms | 2.16 s (277x slower) |

Root cause: 1000 entries × 2.1 ms GPU overhead per dispatch = 2.1 s.

### Batched store scoring — GPU can win

By collecting ALL vectors across ALL entries into contiguous buffers
and dispatching ONE GPU kernel:

- 1000 entries × 32 vectors = 32K vectors
- GPU: 2.1 ms overhead + ~0.5 ms compute ≈ **2.6 ms** (estimated)
- CPU compressed: 7.8 ms

Single-dispatch batch should be **~3x faster** than CPU for 1000+ entries.

## Decision

### GPU only in `scores` batch path

1. **`score()` — always CPU.** Per-entry calls (32 vectors) can never
...
4. **`scores()` — always float x sign.**
   With GPU: batches all sign bits + norms across all entries into
...
- Per-entry scoring is always fast (CPU, no GPU overhead)
- Store-level scoring benefits from GPU when enough entries are present
...
1. **GPU dispatch in `score()` with high threshold** — rejected because
   the threshold (5K+) is never reached by per-entry calls, making
