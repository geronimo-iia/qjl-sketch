...
| `KeysConfig` | `store::config` | dim, sketch_dim, seed, etc. |
| `ValuesConfig` | `store::config` | bits, group_size |
| `IndexEntry` | `store::config` | entry_id, offset, etc. |
| `IndexMeta` | `store::config` | entry_count, live_bytes, dead_bytes |
| `KeyExportEntry` | `store::key_store` | entry_id, content_hash, CompressedKeys |
| `ValueExportEntry` | `store::value_store` | entry_id, content_hash, CompressedValues |
...
|--------|-------------------|---------------|
| `QJLSketch` | dim, sketch_dim, outlier_sketch_dim, seed | proj_dir_score, proj_dir_quant |
...
| `KeyEntryView` / `ValueEntryView` | Borrowed references |
